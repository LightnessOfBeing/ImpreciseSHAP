...
def solve_min(
    problem_type: str, vs: Dict[str, float], k: int, pi: np.ndarray, alpha: np.ndarray
) -> Optional[Tuple[float, float, int]]:
    if problem_type != "l2" and problem_type != "l1":
        raise ValueError("Problem type has to be either l1 or l2!")
    model = LpProblem(name=f"{problem_type}-{k}", sense=LpMinimize)
    B = LpVariable(name="B")
...
def solve_max(
    problem_type: str, vs: Dict[str, float], k: int, tau: np.ndarray, alpha: np.ndarray
) -> Optional[Tuple[float, float, int]]:
    model = LpProblem(name=f"{problem_type}-{k}", sense=LpMaximize)
    B = LpVariable(name="B")
    ...
    if problem_type == "u1":
        ...
        for i in range(C - 1):
            if i != k:
                model += vs[f"pi_u_{k}"] - alphas[k] >= vs[f"pi_u_{i}"] - alphas[i]
    else:
        model += alphas[k] >= vs[f"pi_l_{k}"]
        for i in range(C - 1):
            if i != k:
                model += alphas[k] - vs[f"pi_l_{k}"] >= alphas[i] - vs[f"pi_l_{i}"]
